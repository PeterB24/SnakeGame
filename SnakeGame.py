...
def collision_with_boundaries(snake_head):
    if snake_head[0]>=500 or snake_head[0]<0 or snake_head[1]>=500 or snake_head[1]<0 : #boundaries can be adjusted to any size
        return 1
    else:
        return 0

# game restarts if the snake collides with itself
...
# Checks if the direction snake is travelling is blocked, if so end game
...
def generate_snake(snake_head, snake_position, apple_position, button_direction, score):
    ...
    #score increases if snake hits apple
...
#Main code for the playing of the game 
...
###### initialize required parameters ########  
    ...
    apple_image = pygame.image.load('Apple_icon.png')
